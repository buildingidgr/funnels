...
export const resetFunnels = (): void => {
  ...
  try {
    ...
    // Initialize with default funnel
    mockFunnels = [defaultFunnel];
    console.log('Initialized with default funnel:', defaultFunnel);
    
    // Save to localStorage
    localStorage.setItem(STORAGE_KEYS.FUNNELS, JSON.stringify(mockFunnels));
    console.log('Successfully saved default funnel to localStorage');
  } catch (error) {
  ...
  calculateFunnel: async (id: string): Promise<FunnelResults> => {
    try {
      ...
      funnel.steps.forEach(step => {
        if (step.visitorCount) {
          results[step.id] = step.visitorCount;
        }
      });

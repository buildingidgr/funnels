...
export function StepAccordionList({ steps, onChange }: StepAccordionListProps) {
  ...
  const updateStepConditions = (stepIndex: number, conditions: any[]) => {
    const updatedSteps = [...steps];
  ...
  const updateSplitConditions = (stepIndex: number, splitIndex: number, conditions: any[]) => {
    const updatedSteps = [...steps];
    const step = updatedSteps[stepIndex];
    
    if (step.splitVariations && step.splitVariations[splitIndex]) {
      step.splitVariations[splitIndex].conditions = {
        orEventGroups: conditions
      };
    }
